package com.cooperay.common.core.dao;
...
import com.cooperay.common.core.mybatis.interceptor.ExecutorInterceptor;
...
public abstract class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public static final String SQL_UPDATE_SELECTIVE="updateSelective";
	...
	public long insertSelective(T t){
		...
		int result = sessionTemplate.insert(getStatement(SQL_INSERT_SELECTIVE));
		if(result<=0){
			throw BizException.DB_INSERT_RESULT_0;
		}
		if(t !=null && t.getId()!=null && result > 0 )
